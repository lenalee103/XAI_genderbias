...
def apply_SMOTE(dataset_Binary, dataset_val, privileged_groups, unprivileged_groups):
    ...
    # Train xgboost model
    xgb_model = XGBClassifier(objective='binary:logistic', random_state=42)
    xgb_model.fit(X_train_scaled, y_train_transf)

    # Evaluate model
    y_pred = xgb_model.predict(X_val_scaled)
    print("\nXGBoost Model Performance on Validation Set (SMOTE):")
    print("Accuracy:", accuracy_score(y_val, y_pred))
...
if __name__ == "__main__":
    ...
    print("SMOTE-Adversial Debiasing")
    apply_SMOTE(dataset_transf_train_AdDe, dataset_val, privileged_groups, unprivileged_groups)
